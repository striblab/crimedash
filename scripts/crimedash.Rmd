...
get_crime_data <- function(city_name, ori, output_folder, api_key) {
  # Normalize city name for file naming
  file_city <- str_to_lower(str_replace_all(city_name, "\\s+", "_"))

  # Offense codes according to FBI API
  violent_crimes <- c("HOM", "RPE", "ROB", "ASS")
  property_crimes <- c("BUR", "LAR", "ARS", "MVT")
  ...
  # Define function to get data for a single offense
  get_offense_data <- function(offense) {
    ...
    tryCatch({
      ...
      content(res, as = "parsed", simplifyVector = TRUE)$results %>%
        as_tibble() %>%
        transmute(
